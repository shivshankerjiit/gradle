Ext.override(Ext.tree.TreeNodeUI, {
    ...
    renderElements :function(n, a, targetNode, bulkRender){
        ...
        var buf = ['<li class="x-tree-node"><div ext:tree-node-id="',n.id,'" class="x-tree-node-el x-tree-node-leaf x-unselectable ', a.cls,'" unselectable="on">',
            ...
            cb ? ('<img src="'+this.emptyIcon+'" class="x-tree-checkbox'+(a.checked === true ? ' x-tree-node-checked' :((a.checked !== false || a.hasCheckedChildren === true) ? ' x-tree-node-grayed' :''))+'" />') :'',
            '<a hidefocus="on" class="x-tree-node-anchor" href="',href,'" tabIndex="1" ',
            a.hrefTarget ? ' target="'+a.hrefTarget+'"' :"", '><span unselectable="on">',n.text,"</span></a></div>",
            '<ul class="x-tree-node-ct" style="display:none;"></ul>',
            "</li>"].join('');
        */
        var assignmentType = Ext.getCmp('assignment-combo').getValue();
        var displayText = '';
        if(assignmentType == 'btsk'){
            displayText = n.text;
        } else if(assignmentType == 'mkmsk'){
            displayText =  (n.id.length > 3 ? n.id.substring(3) : n.id) + ' | ' + n.text;
        } else if(assignmentType == 'prodweb'){
            displayText = n.text;
        } else {
            displayText = n.id + ' | ' + n.text;
        }
            ...
            '<li class="x-tree-node">' +
              '<div ext:tree-node-id="', n.id, '" class="x-tree-node-el x-tree-node-leaf x-unselectable ', a.cls, '" unselectable="on">',
                ...
                '<a hidefocus="on" class="x-tree-node-anchor" href="', href, '" tabIndex="1" ', a.hrefTarget ? ' target="' + a.hrefTarget + '"' : "", '>',
                  (a.gueltigkeitsstatus == 1 || a.gueltigkeitsstatus == 3) ? "<span class='x-tree-node-status'>?</span>" : '',
...
Ext.override(Ext.tree.TreePanel, {
    ...
    /**
     * @param onlyParents {Boolean} true if only the parent checks should be returned
     */
    getChecked: function(a, startNode, onlyParents) {
        startNode = startNode || this.root;
        var r = [];
        var f = function() {
            if (this.ui.getChecked(true)) {
    ...
    findFirstChild: function(a, value, deep, startNode){
        startNode = startNode || this.root;
        var node,
            ...
            i = 0;
        for(;i < len; i++){
            if(children[i].attributes[a] == value){
                return children[i];
            } else if(deep) {
                var res = this.findFirstChild(a, value, deep, children[i]);
